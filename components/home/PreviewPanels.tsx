...
export function ConvertSummaryPanel({ files, selectedFile, targetFormat }: ConvertSummaryPanelProps) {
  return (
  ...
  return (
    <div className="rounded-2xl border bg-white p-4">
      ...
      <div className="mt-4 text-xs text-slate-500">
        Original {formatBytes(props.sourceSizeBytes)} {"->"} Compressed {formatBytes(props.outputSizeBytes)}
      </div>
    </div>
  );
}
...
export function CropEditorPanel({
  ...
  return (
    <div className="rounded-2xl border bg-white p-4">
      ...
      <div className="relative flex aspect-video items-center justify-center overflow-hidden rounded-xl border bg-slate-100">
        {!preview ? (
...
export function ResizePreviewPanel({ preview, previewError, width, height }: ResizePreviewPanelProps) {
  return (
    <div className="rounded-2xl border bg-white p-4">
      <div className="mb-3 flex items-center justify-between">
        <div className="font-medium">Resize output</div>
        <div className="text-xs text-slate-500">Export uses the exact target size</div>
      </div>
      ...
      <div className="mt-4 grid grid-cols-2 gap-3 text-sm">
        <div className="rounded-xl border bg-slate-50 p-3">
          <div className="text-xs uppercase tracking-[0.18em] text-slate-500">Width</div>
          <div className="mt-2 text-2xl font-semibold text-slate-900">{width || "--"}</div>
        </div>
        <div className="rounded-xl border bg-slate-50 p-3">
          <div className="text-xs uppercase tracking-[0.18em] text-slate-500">Height</div>
          <div className="mt-2 text-2xl font-semibold text-slate-900">{height || "--"}</div>
        </div>
      </div>
    </div>
  );
